...
entity fifo_sync is
  generic (
    ADV_FEATURES : string  := "1000"; -- enable data_valid
    DEPTH        : integer := 512;
    WR_WIDTH     : integer := 64;
    RD_WIDTH     : integer := 64;
    WR_CNT_WIDTH : integer := integer(ceil(log2(real(DEPTH))));
    RD_CNT_WIDTH : integer := integer(ceil(log2(real(DEPTH)*real(WR_WIDTH)/real(RD_WIDTH))));
  ...
  port (
    ...
    wr_cnt  : out std_logic_vector(WR_CNT_WIDTH-1 downto 0);
    rd_cnt  : out std_logic_vector(RD_CNT_WIDTH-1 downto 0);
    valid   : out std_logic;
    full    : out std_logic;
    empty   : out std_logic
    );
...
begin
  ...
  xpm_fifo_sync_inst : xpm_fifo_sync
    generic map (
      ...
      RD_DATA_COUNT_WIDTH => RD_CNT_WIDTH, -- DECIMAL
    ...
    port map (
      ...
      rd_data_count => rd_cnt, -- RD_DATA_COUNT_WIDTH-bit output: Read Data Count: This bus indicates the number of words read from the FIFO.
      ...
      wr_data_count => wr_cnt, -- WR_DATA_COUNT_WIDTH-bit output: Write Data Count: This bus indicates the number of words written into the FIFO.
