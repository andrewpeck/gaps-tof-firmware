...
% START: ADDRESS_TABLE :: DO NOT EDIT
    ...
    TRACK\_CENTRAL\_IS\_GLOBAL & \texttt{0xb} & \texttt{[2:2]} & rw & \texttt{0x0} & 1 makes the TRACK central read all paddles. \\\hline
    TRACK\_UMB\_CENTRAL\_IS\_GLOBAL & \texttt{0xb} & \texttt{[3:3]} & rw & \texttt{0x0} & 1 makes the TRACK UMB central read all paddles. \\\hline
    ...
    TIU\_BUSY\_STUCK & \texttt{0xf} & \texttt{[1:1]} & r & \texttt{} & 1 means the TIU has been stuck high for a long time \\\hline
    TIU\_BUSY\_IGNORE & \texttt{0xf} & \texttt{[2:2]} & rw & \texttt{0x0} & 1 means the the MTB should ignore the TIU busy flag (e.g. because it is stuck) \\\hline
    MIN\_DEADTIME\_MODE & \texttt{0xf} & \texttt{[3:3]} & rw & \texttt{0x0} & Minimize deadtime by ignoring the TIU module. \\\hline
    LT\_INPUT\_STRETCH & \texttt{0xf} & \texttt{[7:4]} & rw & \texttt{0xF} & Number of clock cycles to stretch the LT inputs by \\\hline
    RB\_INTEGRATION\_WINDOW & \texttt{0xf} & \texttt{[12:8]} & rw & \texttt{0x1} & Number of 100MHz clock cycles to integrate the LTB hits to determine which RBs to read out. \\\hline
    ...
    NUM\_EVENTS & \texttt{0x13} & \texttt{[13:0]} & r & \texttt{} & DAQ Buffer Number of Events \\\hline
    ...
    TOTAL\_TOF\_THRESH & \texttt{0x14} & \texttt{[23:16]} & rw & \texttt{0x8} & Total TOF hit threshold \\\hline
    ...
    CONFIGURABLE\_TRIGGER\_EN & \texttt{0x14} & \texttt{[31:31]} & rw & \texttt{0x0} & Enable the configurable trigger \\\hline
    CUBE\_SIDE\_THRESH & \texttt{0x15} & \texttt{[7:0]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    CUBE\_TOP\_THRESH & \texttt{0x15} & \texttt{[15:8]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    CUBE\_BOT\_THRESH & \texttt{0x15} & \texttt{[23:16]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    CUBE\_CORNER\_THRESH & \texttt{0x15} & \texttt{[31:24]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    UMBRELLA\_THRESH & \texttt{0x16} & \texttt{[7:0]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    UMBRELLA\_CENTER\_THRESH & \texttt{0x16} & \texttt{[15:8]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    CORTINA\_THRESH & \texttt{0x16} & \texttt{[23:16]} & rw & \texttt{0x0} & Threshold for the hit bitmap. Threshold must be > this number. \\\hline
    ...
    TRACK\_CENTRAL\_PRESCALE & \texttt{0x42} & \texttt{[31:0]} & rw & \texttt{0x0} & Prescale value for the Umbrella + Cube Top Track Trigger. 0 == 0\% (off), 2**32-1 == 100\% \\\hline
    FORCE\_TRIGGER\_PRESCALE & \texttt{0x43} & \texttt{[31:0]} & rw & \texttt{0xFFFFFFFF} & Prescale value for the force trigger \\\hline
    PRESCALE\_BYPASS & \texttt{0x44} & \texttt{[0:0]} & rw & \texttt{0x1} & 1 to bypass prescalers \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    \noindent
    \subsection*{\textcolor{parentcolor}{\textbf{MT.PULSER}}}
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    FIRE & \texttt{0x100} & \texttt{[0:0]} & w & Pulse & Write 1 to Fire the Pulser. \\\hline
    CH\_0\_24 & \texttt{0x101} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 0 to 24 \\\hline
    CH\_25\_49 & \texttt{0x102} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 25 to 49 \\\hline
    CH\_50\_74 & \texttt{0x103} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 50 to 74 \\\hline
    CH\_75\_99 & \texttt{0x104} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 75 to 99 \\\hline
    CH\_100\_124 & \texttt{0x105} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 100 to 124 \\\hline
    CH\_125\_149 & \texttt{0x106} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 125 to 149 \\\hline
    CH\_150\_174 & \texttt{0x107} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 150 to 174 \\\hline
    CH\_175\_199 & \texttt{0x108} & \texttt{[24:0]} & rw & \texttt{0x0} & Set fire bits for channels 175 to 199 \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    TIU\_BUSY\_LENGTH & \texttt{0x11f} & \texttt{[31:0]} & r & \texttt{} & Length in 10ns cycles of the last TIU busy flag \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    TRIG\_CYCLIC\_EN & \texttt{0x240} & \texttt{[0:0]} & rw & \texttt{0x0} & Interval of cyclic triggers \\\hline
    TRIG\_CYCLIC\_INTERVAL & \texttt{0x241} & \texttt{[31:0]} & rw & \texttt{0x0} & Interval of cyclic triggers \\\hline
    LT\_LINK\_EN0 & \texttt{0x242} & \texttt{[9:0]} & rw & \texttt{0x3FF} & DSI 0 RX Link Enable \\\hline
    LT\_LINK\_EN1 & \texttt{0x243} & \texttt{[9:0]} & rw & \texttt{0x3FF} & DSI 1 RX Link Enable \\\hline
    LT\_LINK\_EN2 & \texttt{0x244} & \texttt{[9:0]} & rw & \texttt{0x3FF} & DSI 2 RX Link Enable \\\hline
    LT\_LINK\_EN3 & \texttt{0x245} & \texttt{[9:0]} & rw & \texttt{0x3FF} & DSI 3 RX Link Enable \\\hline
    LT\_LINK\_EN4 & \texttt{0x246} & \texttt{[9:0]} & rw & \texttt{0x3FF} & DSI 4 RX Link Enable \\\hline
    LT\_LINK\_AUTOMASK & \texttt{0x247} & \texttt{[0:0]} & rw & \texttt{0x1} & 1 to enable automatic LT link masking \\\hline
    GAPS\_TRIG\_PRESCALE & \texttt{0x248} & \texttt{[31:0]} & rw & \texttt{0xFFFFFFFF} & Prescale value for the GAPS trigger. 0 == 0\% (off), 2**32-1 == 100\% \\\hline
    TRACK\_UMB\_CENTRAL\_PRESCALE & \texttt{0x249} & \texttt{[31:0]} & rw & \texttt{0x0} & Prescale value for the Umbrella Center + Cube Top Track Trigger. 0 == 0\% (off), 2**32-1 == 100\% \\\hline
    RB\_BLOCK\_IF\_BUSY\_31\_TO\_0 & \texttt{0x24a} & \texttt{[31:0]} & rw & \texttt{0x0} & Bitmask to specify if a readout board is BUSY then do not trigger (RB slots 31:0) \\\hline
    RB\_BLOCK\_IF\_BUSY\_49\_TO\_32 & \texttt{0x24b} & \texttt{[17:0]} & rw & \texttt{0x0} & Bitmask to specify if a readout board is BUSY then do not trigger (RB slots 49:32) \\\hline
    RB\_LOST\_TRIGGER\_RATE & \texttt{0x24c} & \texttt{[23:0]} & r & \texttt{} & Rate of lost triggers due to RB deadtime (in Hz) \\\hline
    TIU\_LOST\_TRIGGER\_RATE & \texttt{0x24d} & \texttt{[23:0]} & r & \texttt{} & Rate of lost triggers due to TIU deadtime (in Hz) \\\hline
    TRG\_LOST\_TRIGGER\_RATE & \texttt{0x24e} & \texttt{[23:0]} & r & \texttt{} & Rate of lost triggers due to MTB internal trigger block deadtime (in Hz) \\\hline
    GAPS\_TRIGGER\_BLOCKED\_RATE & \texttt{0x24f} & \texttt{[23:0]} & r & \texttt{} & Rate of this trigger blocked due to prescaler or disable \\\hline
    TRACK\_TRIGGER\_BLOCKED\_RATE & \texttt{0x250} & \texttt{[23:0]} & r & \texttt{} & Rate of this trigger blocked due to prescaler or disable \\\hline
    ANY\_TRIGGER\_BLOCKED\_RATE & \texttt{0x251} & \texttt{[23:0]} & r & \texttt{} & Rate of this trigger blocked due to prescaler or disable \\\hline
    TRACK\_CENTRAL\_BLOCKED\_RATE & \texttt{0x252} & \texttt{[23:0]} & r & \texttt{} & Rate of this trigger blocked due to prescaler or disable \\\hline
    TRACK\_UMB\_CENTRAL\_BLOCKED\_RATE & \texttt{0x253} & \texttt{[23:0]} & r & \texttt{} & Rate of this trigger blocked due to prescaler or disable \\\hline
    TIU\_BUSY\_RATE & \texttt{0x254} & \texttt{[23:0]} & r & \texttt{} & Rate of TIU asserting busy. Measures the fraction of time the TIU was busy. \\\hline
    \end{tabularx}
    \vspace{5mm}
