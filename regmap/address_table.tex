...
% START: ADDRESS_TABLE :: DO NOT EDIT
    ...
    READOUT\_MASK & \texttt{0x11} & \texttt{[8:0]} & rw & \texttt{0x1FF} & 8 bit mask, set a bit to 1 to enable readout of that channel. 9th is auto-read if any channel is enabled \\\hline
    ...
    WAIT\_VDD\_CLKS & \texttt{0x18} & \texttt{[15:0]} & rw & \texttt{0x4000} & Number of 30 ns clocks to wait before reading out the drs, allowing vdd to stabilize \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    EXT\_TRIGGER\_ACTIVE\_HI & \texttt{0x41} & \texttt{[1:1]} & rw & \texttt{0x1} & Generates a trigger \\\hline
    \end{tabularx}
    \vspace{5mm}
