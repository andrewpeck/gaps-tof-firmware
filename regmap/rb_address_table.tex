...
% START: ADDRESS_TABLE :: DO NOT EDIT
    ...
    RAT\_HOUSEKEEPING & \texttt{0x2c} & \texttt{[31:0]} & rw & \texttt{0x0} & 32 bit RAT housekeeping data. Meaning is software defined. \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    DAQ\_FRAGMENT\_EN & \texttt{0x31} & \texttt{[0:0]} & rw & \texttt{0x0} & 1 to enable daq fragments (header only packets) when the DRS is busy \\\hline
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    CNT\_SNAP & \texttt{0x80} & \texttt{[0:0]} & w & Pulse & 1 to freeze counters for readout \\\hline
    CNT\_SNAP\_DIS & \texttt{0x80} & \texttt{[1:1]} & w & \texttt{0x1} & 1 disables counter snapping; values continuously update \\\hline
    CNT\_START\_READOUT & \texttt{0x81} & \texttt{[31:0]} & r & \texttt{} & Count the number of times the DRS readout has started \\\hline
    CNT\_EVENT\_WR\_EN & \texttt{0x82} & \texttt{[31:0]} & r & \texttt{} & Count the number of times the event queue has been written to \\\hline
    AUTO\_PURGE\_MODE & \texttt{0x83} & \texttt{[0:0]} & rw & \texttt{0x0} & 1 for the DAQ to automatically purge any remaining data from the DRS fifo after readout \\\hline
    \end{tabularx}
    \vspace{5mm}
