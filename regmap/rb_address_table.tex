...
% START: ADDRESS_TABLE :: DO NOT EDIT
    ...
    TRIGGER\_DELAY & \texttt{0x42} & \texttt{[11:0]} & rw & \texttt{0x0} & Trigger delay measured in LUT1 units \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    TRIG\_GEN\_RATE & \texttt{0x59} & \texttt{[31:0]} & rw & \texttt{0x0} & Rate of generated triggers f\_trig =(2\^32-1) * clk\_period * rate \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    EVENTID\_TIMEOUT\_CNT & \texttt{0x202} & \texttt{[15:0]} & r & \texttt{} & Timed out triggers waiting for event id \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    PL\_MMCM\_LOCKED & \texttt{0x1000} & \texttt{[0:0]} & r & \texttt{} & PL MMCM is locked \\\hline
    IPB\_CLK\_RATE & \texttt{0x1001} & \texttt{[31:0]} & r & \texttt{} & Rate of IPB Clk in Hz \\\hline
    CNT\_DMA\_READOUTS\_COMPLETED & \texttt{0x1010} & \texttt{[31:0]} & r & \texttt{} & Number of readouts completed since reset \\\hline
    \end{tabularx}
    \vspace{5mm}


    \noindent
    \subsection*{\textcolor{parentcolor}{\textbf{PS.DMA}}}
    ...
    \textbf{Node} & \textbf{Adr} & \textbf{Bits} & \textbf{Perm} & \textbf{Def} & \textbf{Description} \\\hline
    \nopagebreak
    RAM\_A\_OCC\_RST & \texttt{0x1100} & \texttt{[0:0]} & w & Pulse & Sets RAM buffer a counter to 0 \\\hline
    RAM\_B\_OCC\_RST & \texttt{0x1101} & \texttt{[0:0]} & w & Pulse & Sets RAM buffer b counter to 0 \\\hline
    RAM\_A\_OCCUPANCY & \texttt{0x1102} & \texttt{[31:0]} & r & \texttt{} & RAM buffer a occupancy \\\hline
    RAM\_B\_OCCUPANCY & \texttt{0x1103} & \texttt{[31:0]} & r & \texttt{} & RAM buffer b occupancy \\\hline
    DMA\_POINTER & \texttt{0x1104} & \texttt{[31:0]} & r & \texttt{} & DMA controller pointer \\\hline
    TOGGLE\_RAM & \texttt{0x1105} & \texttt{[0:0]} & w & Pulse & Write 1 to switch the dma buffer to the other half \\\hline
    DMA\_CLEAR & \texttt{0x1106} & \texttt{[0:0]} & w & Pulse & Write 1 to clear the DMA memory (write zeroes) \\\hline
    DMA\_RESET & \texttt{0x1107} & \texttt{[0:0]} & w & Pulse & Write 1 to completely reset the DMA state machine logic \\\hline
    \end{tabularx}
    \vspace{5mm}
