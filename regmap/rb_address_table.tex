...
% START: ADDRESS_TABLE :: DO NOT EDIT
    ...
    CLK\_IDELAY & \texttt{0x2} & \texttt{[4:0]} & rw & \texttt{0x1F} & DRS CLK IDELAY Setting 0-31 in 156 ps increments \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    DAQ\_FRAGMENT\_EN & \texttt{0x30} & \texttt{[0:0]} & w & \texttt{0x1} & 1 to enable daq fragments (header only packets) when the DRS is busy \\\hline
    \end{tabularx}
    \vspace{5mm}
    ...
    MT\_TRIGGER\_MODE & \texttt{0x45} & \texttt{[0:0]} & w & \texttt{0x1} & 1 to use the MT as the source of the trigger \\\hline
